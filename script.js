...
function initLanguage() {
  // 1. Inject Language Button into Sidebar
  ...
  const i18n = {
    en: {
      ...
      'nav-unit': 'Units', 'nav-bmi': 'BMI', 'nav-tip': 'Tip', 'nav-age': 'Age', 'nav-loan': 'Loan/EMI',
      ...
      'title-age': 'Age Calculator', 'title-loan': 'Loan / EMI',
    ...
    ur: {
      ...
      'nav-unit': 'یونٹس', 'nav-bmi': 'بی ایم آئی', 'nav-tip': 'ٹپ', 'nav-age': 'عمر', 'nav-loan': 'قرضہ',
      ...
      'title-age': 'عمر کیلکولیٹر', 'title-loan': 'قرضہ / ای ایم آئی',
...
function initCurrency() {
  ...
  Object.entries(FX).forEach(([code,info])=>{
    [fromSel,toSel].forEach(sel=>{ 
      ...
      o.value=code; 
      o.textContent=`${info.f} ${code} — ${info.n}`; 
      sel.appendChild(o); 
    });
  });
...
function initBMI() {
  ...
  calcBtn.addEventListener('click',()=>{
    ...
    if(!h||!w||h<=0||w<=0){ toast('Enter valid values'); return; }
    bmi=parseFloat((w/(h*h)).toFixed(1)); numEl.textContent=bmi;
    const cat=cats.find(c=>bmi<c.max)||cats[3]; catEl.textContent=cat.label; catEl.style.color=cat.color;
    const pct=Math.min(100,Math.max(0,((bmi-10)/30)*100)); fillEl.style.width=(100-pct)+'%'; markerEl.style.left=pct+'%';
    idealEl.textContent=`Ideal weight: ${(18.5*h*h).toFixed(1)}–${(24.9*h*h).toFixed(1)} ${isMetric?'kg':'lbs'}`;
...
function initAge() {
  ...
  btn.addEventListener('click',()=>{
    ...
    if(isNaN(dob)||isNaN(to)||dob>=to){ toast('Invalid dates'); return; }
    ...
    [{n:years,l:'Years'},{n:months,l:'Months'},{n:days,l:'Days'},{n:Math.floor(totalDays/7),l:'Weeks'},{n:(totalDays*24).toLocaleString(),l:'Hours'},{n:(totalDays*24*60).toLocaleString(),l:'Minutes'}].forEach(c=>{
      const d=document.createElement('div'); d.className='age-cell'; d.innerHTML=`<div class="age-num">${c.n}</div><div class="age-lbl">${c.l}</div>`; grid.appendChild(d);
    });
...
function initLoan() {
  ...
  btn.addEventListener('click',()=>{
    ...
    if(!P||!years){ toast('Enter valid values'); return; }
    ...
    [{l:'Principal',v:'$'+P.toLocaleString()},{l:'Interest',v:'$'+totalInt.toLocaleString('en-US',{maximumFractionDigits:0})},{l:'Total',v:'$'+totalPay.toLocaleString('en-US',{maximumFractionDigits:0})},{l:'Term',v:`${years} Years`}].forEach(it=>{
      const d=document.createElement('div'); d.className='lb-item'; d.innerHTML=`<div class="lb-label">${it.l}</div><div class="lb-val">${it.v}</div>`; bd.appendChild(d);
    });
...
function drawDonut(principal,interest) {
  ...
  arc(-Math.PI/2,pAngle,'#00d4ff'); arc(pAngle,3*Math.PI/2,'#ffaa00');
  ctx.fillStyle='#f0f4ff'; ctx.font='bold 13px Space Mono'; ctx.textAlign='center'; ctx.fillText('TOTAL',cx,cy-6);
  ctx.font='bold 11px Space Mono'; ctx.fillText('$'+total.toLocaleString('en-US',{maximumFractionDigits:0}),cx,cy+10);
  document.getElementById('chart-legend').innerHTML=`<div class="cl-item"><div class="cl-dot" style="background:#00d4ff"></div><span>Principal</span></div><div class="cl-item"><div class="cl-dot" style="background:#ffaa00"></div><span>Interest</span></div>`;
}
